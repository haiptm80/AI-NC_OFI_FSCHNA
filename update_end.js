        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity">
          <div className="bg-white rounded-sm shadow-xl w-full max-w-5xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center shrink-0">
               <span className="text-sm font-bold text-slate-600 bg-white border border-slate-200 px-3 py-1 rounded-sm shadow-sm uppercase tracking-widest">
                  Đã lọc: {drillDownModal.data.length} bản ghi
               </span>
               <span className="text-xs text-slate-500 font-medium">
                  Chỉ hiển thị dữ liệu liên quan đến mục bạn đã chọn.
               </span>
            </div>
            <div className="overflow-auto custom-scrollbar flex-1 p-0 m-0">
               {drillDownModal.data.length > 0 ? (
                 ...
                 <div className="p-10 text-center text-slate-500 flex flex-col items-center justify-center h-full gap-4">
                   <Info className="w-12 h-12 text-slate-300" />
                   <p className="text-sm font-medium">Không có dữ liệu phù hợp với phân loại này.</p>
                 </div>
               )}
            </div>
          </div>
