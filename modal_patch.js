...
const JS_FILE = 'src/App.tsx';
let data = fs.readFileSync(JS_FILE, 'utf8');

data = data.replace(
  'onClick={(data) => setFilters(prev => ({ ...prev, category: (data as any)?.name || prev.category }))}',
  'onClick={(data) => { const category = (data as any)?.name; if (!category) return; const filteredRows = chartData.filtered.filter((r: any) => String(r[dataKeys.categoryKey]) === category); setDrillDownModal({ isOpen: true, title: "Loại lỗi: " + category, data: filteredRows }); }}'
);

data = data.replace(
  'onClick={() => setFilters(prev => ({ ...prev, dept: d }))}',
  'onClick={() => { const filteredRows = chartData.filtered.filter((r: any) => String(r[dataKeys.deptKey]) === d); setDrillDownModal({ isOpen: true, title: "Phòng ban: " + d, data: filteredRows }); }}'
);

data = data.replace(
  'setFilters(prev => ({ ...prev, severity: entry.name || prev.severity }));',
  'const filteredRows = chartData.filtered.filter((r: any) => String(r[dataKeys.severityKey]).trim() === entry.name); setDrillDownModal({ isOpen: true, title: "Mức độ rủi ro: " + entry.name, data: filteredRows });'
);

const modalJsx = \`      {drillDownModal.isOpen && (
...
data = data.replace('{/* Header FPT Education */}', modalJsx + '\\n      {/* Header FPT Education */}');

fs.writeFileSync(JS_FILE, data);
